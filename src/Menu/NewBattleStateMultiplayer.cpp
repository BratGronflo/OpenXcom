 ...
 */
#include "NewBattleStateMultiplayer.h"
#include <cmath>
#include <algorithm>
...
namespace OpenXcom
{
	...
	/**
	 * Initializes all the elements in the New Multiplayer Battle window.
	 * @param game Pointer to the core game.
	 */
	NewBattleStateMultiplayer::NewBattleStateMultiplayer() : _craft(0), _selectType(NewBattleSelectType::MISSION), _isRightClick(false)
	{
		//Create objects
		...
		_btnJoin = new TextButton(200, 32, 150, 200);
		...
		add(_btnJoin, "button2", "newBattleMenu");
		...
		_cbxMission->onChange((ActionHandler)&NewBattleStateMultiplayer::cbxMissionChange);
		...
		_cbxCraft->onChange((ActionHandler)&NewBattleStateMultiplayer::cbxCraftChange);

		_slrDarkness->setRange(0, 15);

		_slrDepth->setRange(1, 3);

		_cbxTerrain->onChange((ActionHandler)&NewBattleStateMultiplayer::cbxTerrainChange);
		...
		_btnEquip->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnEquipClick);

		_btnRandom->setText(tr("STR_RANDOMIZE"));
		_btnRandom->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnRandomClick);

		_btnOk->setText(tr("STR_OK"));
		_btnOk->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnOkClick);
		_btnOk->onKeyboardPress((ActionHandler)&NewBattleStateMultiplayer::btnOkClick, Options::keyOk);

		_btnJoin->setText(tr("STR_JOIN"));
		_btnJoin->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnJoinClick);

		_btnCancel->setText(tr("STR_CANCEL"));
		_btnCancel->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnCancelClick);
		_btnCancel->onKeyboardPress((ActionHandler)&NewBattleStateMultiplayer::btnCancelClick, Options::keyCancel);
		...
		_btnMission->setText("...");
		_btnMission->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnMissionChange);
		_btnMission->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnMissionChange, SDL_BUTTON_RIGHT);
		_btnMission->setVisible(showExtraButtons);

		_btnTerrain->setText("...");
		_btnTerrain->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnTerrainChange);
		_btnTerrain->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnTerrainChange, SDL_BUTTON_RIGHT);
		_btnTerrain->setVisible(showExtraButtons);

		_btnAlienRace->setText("...");
		_btnAlienRace->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnAlienRaceChange);
		_btnAlienRace->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::btnAlienRaceChange, SDL_BUTTON_RIGHT);
		...
		_lstSelect->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::lstSelectClick);
		_lstSelect->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::lstSelectClick, SDL_BUTTON_RIGHT);
		_lstSelect->onMouseClick((ActionHandler)&NewBattleStateMultiplayer::lstSelectClick, SDL_BUTTON_MIDDLE);
		_lstSelect->setVisible(false);

		_btnQuickSearch->setText(""); // redraw
		_btnQuickSearch->onEnter((ActionHandler)&NewBattleStateMultiplayer::btnQuickSearchApply);
		_btnQuickSearch->setVisible(false);

		_btnCancel->onKeyboardRelease((ActionHandler)&NewBattleStateMultiplayer::btnQuickSearchToggle, Options::keyToggleQuickSearch);
}

/**
 *
 */
NewBattleStateMultiplayer::~NewBattleStateMultiplayer()
{
 ...
 */
void NewBattleStateMultiplayer::init()
{
	State::init();
 ...
 */
void NewBattleStateMultiplayer::load(const std::string& filename)
{
	...
	else
	{
		try
		{
			...
			_cbxMission->setSelected(min(doc["mission"].as<size_t>(0), _missionTypes.size() - 1));
			cbxMissionChange(0);
			_cbxCraft->setSelected(min(doc["craft"].as<size_t>(0), _crafts.size() - 1));
			_slrDarkness->setValue(doc["darkness"].as<size_t>(0));
			_cbxTerrain->setSelected(min(doc["terrain"].as<size_t>(0), _terrainTypes.size() - 1));
			cbxTerrainChange(0);
			_cbxAlienRace->setSelected(min(doc["alienRace"].as<size_t>(0), _alienRaces.size() - 1));
 ...
 * @param filename YAML filename.
 */
void NewBattleStateMultiplayer::save(const std::string& filename)
{
	YAML::Emitter out;
	YAML::Node node;
 ...
 */
void NewBattleStateMultiplayer::initSave()
{
	const Mod* mod = _game->getMod();
	SavedGame* save = new SavedGame();
	Base* base = new Base(mod);
 ...
 * @param action Pointer to an action.
 */
void NewBattleStateMultiplayer::btnCancelClick(Action*)
{
 ...
 * @param action Pointer to an action.
 */
void NewBattleStateMultiplayer::btnRandomClick(Action*)
{
	initSave();
 ...
 * @param action Pointer to an action.
 */
void NewBattleStateMultiplayer::btnEquipClick(Action*)
{
 ...
 * @param action Pointer to an action.
 */
void NewBattleStateMultiplayer::cbxMissionChange(Action*)
{
 ...
 * @param action Pointer to an action.
 */
void NewBattleStateMultiplayer::cbxCraftChange(Action*)
{
 ...
 * @param action Pointer to an action.
 */
void NewBattleStateMultiplayer::cbxTerrainChange(Action*)
{
	AlienDeployment* ruleDeploy = _game->getMod()->getDeployment(_missionTypes[_cbxMission->getSelected()]);
	int minDepth = 0;
	int maxDepth = 0;
 ...
 */
void NewBattleStateMultiplayer::btnMissionChange(Action* action)
{
 ...
 */
void NewBattleStateMultiplayer::btnTerrainChange(Action* action)
{
 ...
 */
void NewBattleStateMultiplayer::btnAlienRaceChange(Action* action)
{
 ...
 */
void NewBattleStateMultiplayer::fillList(NewBattleSelectType selectType, bool isRightClick)
{
	...
	auto fill = [&](const std::vector<std::string>& list, bool prefix, size_t scroll)
	{
		for (auto& m : list)
		{
			...
			{
				std::string itemName;
				if (_isRightClick) { itemName = m; }
				else { itemName = tr(prefix ? "MAP_" + m : m); }
 ...
 */
void NewBattleStateMultiplayer::lstSelectClick(Action* action)
{
 ...
 */
void NewBattleStateMultiplayer::cleanup()
{
 ...
 * @param action Pointer to an action.
 */
void NewBattleStateMultiplayer::btnQuickSearchToggle(Action* action)
{
	if (_surfaceBackup.empty())
	{
		return;
 ...
 * @param action Pointer to an action.
 */
void NewBattleStateMultiplayer::btnQuickSearchApply(Action*)
{
