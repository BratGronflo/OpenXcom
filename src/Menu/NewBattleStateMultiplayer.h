...
namespace OpenXcom
{
	 ...
	 * standalone mission.
	 */
	class NewBattleStateMultiplayer : public State
	{
	private:
		Window *_window;
		...
		TextButton *_btnOk, *_btnCancel, *_btnEquip, *_btnRandom, *_btnJoin;
		...
		void cleanup();
	public:
		/// Creates the New Battle Multiplayer state.
		NewBattleStateMultiplayer();
		/// Cleans up the New Battle Multiplayer state.
		~NewBattleStateMultiplayer();
		/// Resets state.
		void init() override;
		...
		void btnJoinClick(Action* action);
